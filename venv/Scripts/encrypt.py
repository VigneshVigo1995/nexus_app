...
####################################################  USER INPUT AUDIT REPORT  #############################################################
	...
	df_h2 = d.groupby(['GDS Rate Codes','Multi Rate Code','Sabre','Worldspan','Amaedus','Galileo','Web']).size().reset_index(name='Freq')
...
####################################################  USER INPUT  #############################################################
	###########################################################################ROH############################################	
	list_CE=['Austria','Croatia','Czech Republic','Germany','Hungary','Luxembourg','Slovakia','Slovenia','Switzerland']
	CE_ROH_NUM=[]
	for i in list_CE:
		yy=df_Main.loc[(df_Main['CountryName'] == i )&  (df_Main['RoomDescription'] == 'ROH' )& (df_Main['RoomTypeName'] == 'STANDARD')]["CRSHotelID"]
		lo=yy.tolist()
	...
	for i in CE_ROH_NUM:
		try:
			df_Main=df_Main.drop(df_Main[(df_Main['CRSHotelID'] == i) & (df_Main['RoomTypeName'] == 'DELUXE')].index,inplace=False)
		except:
			pass
	...
	d=d.iloc[1:]
	d = d[["Corp Acct#","GDS Rate Codes","Multi Rate Code","Resort","BWI Rate Code","Map to ROH","Begin Date","End Date","Sabre","Worldspan","Amaedus","Galileo","Web"]]
	d.columns = ["Corp Acct #","GDS Rate Codes","Multi Rate Codes","Resort","BWI Rate Code","Map to ROH (Y/N)","Begin Date","End Date","Sabre","WorldSpan","Amadeus","Galileo","Web"]
	...
	uz_con=['Austria','Croatia','Czech Republic','Germany','Hungary','Luxembourg','Slovakia','Slovenia','Switzerland','Algeria','France','French Guiana','Morocco','Reunion','Channel Islands','England','Gibraltar','Scotland','Wales']
	for i in uz_con:
		df_Cancel.loc[df_Cancel['CountryName'] == i, 'Base Rate'] = 'UZ'
	list_FE=['Algeria','France','French Guiana','Morocco','Reunion']
	FE_ROH_NUM=[]
	for i in list_FE:
		df_Cancel.loc[(df_Cancel['CountryName'] == i)&(df_Cancel['CompBreakfast'] == 'Y')&(df_Cancel['CityTaxIncluded'] == 'N'),'Base Rate'] = '8A'
		df_Cancel.loc[(df_Cancel['CountryName'] == i)&(df_Cancel['CompBreakfast'] == 'N')&(df_Cancel['CityTaxIncluded'] == 'N'),'Base Rate'] = 'RACK'
	...
	for i in range(10):
		...
		if seafd[i].empty:
			...
			if ol=='0':
				refd[i].loc[dfFMS[i]['RoomTypeName'] == 'STANDARD', '1 Person Rate'] = np.repeat(seafd[i]['BD'+str(i+1)+'_RT1_SGL'], lensfs[i])
				refd[i].loc[dfFMS[i]['RoomTypeName'] != 'STANDARD', '1 Person Rate'] = np.repeat(seafd[i]['BD'+str(i+1)+'_RT2_SGL'], lensfs[i])
				refd[i].loc[dfFMS[i]['RoomTypeName'] == 'STANDARD', '2 Person Rate'] = 0
				refd[i].loc[dfFMS[i]['RoomTypeName'] != 'STANDARD', '2 Person Rate'] = 0
			if ol=='1':
				refd[i].loc[dfFMS[i]['RoomTypeName'] == 'STANDARD', '1 Person Rate'] = 0
				refd[i].loc[dfFMS[i]['RoomTypeName'] != 'STANDARD', '1 Person Rate'] = 0
				refd[i].loc[dfFMS[i]['RoomTypeName'] == 'STANDARD', '2 Person Rate'] = np.repeat(seafd[i]['BD'+str(i+1)+'_RT1_DBL'], lensfs[i])
				refd[i].loc[dfFMS[i]['RoomTypeName'] != 'STANDARD', '2 Person Rate'] = np.repeat(seafd[i]['BD'+str(i+1)+'_RT2_DBL'], lensfs[i])
			if ol=='2':
				refd[i].loc[dfFMS[i]['RoomTypeName'] == 'STANDARD', '1 Person Rate'] = np.repeat(seafd[i]['BD'+str(i+1)+'_RT1_SGL'], lensfs[i])
				refd[i].loc[dfFMS[i]['RoomTypeName'] != 'STANDARD', '1 Person Rate'] = np.repeat(seafd[i]['BD'+str(i+1)+'_RT2_SGL'], lensfs[i])
				refd[i].loc[dfFMS[i]['RoomTypeName'] == 'STANDARD', '2 Person Rate'] = np.repeat(seafd[i]['BD'+str(i+1)+'_RT1_DBL'], lensfs[i])
	...
	for i in range(int(rr)):
		if df_Blackout.empty:
			...
			bd_id.append(pd.DataFrame({'CRSHotelID':seabd[i]['CRSHotelID'][~seabd[i]['CRSHotelID'].isin(df_FairDates['CRSHotelID'])].dropna()}))
			#print(bd_id[i])
			...
			if seabd[i].empty:
				...
				rebd.append(pd.DataFrame({'Resort': np.repeat(bd_fd[i]['CRSHotelID'], lensbs[i]),
				            ...
				            'Begin Date': np.repeat(bd_bd[i]['BlackoutDateFrom_'+str(i+1)], lensbs[i]),
	...
	#print(RI)
    ############################################################################################## Cancellation Policy #############################################################################################
	Cancel_Policy = pd.DataFrame({"Resort":df_Cancel['CRSHotelID'],"BWI Rate Code":df_Main['BWIRateCode'],'Begin Date': today,'End Date': '12/31/2099',"Guarantee Policy":'GTD',"Cancel Code":df_Cancel['Cancellation'],"Base Rate":df_Cancel['Base Rate']})
	idx = 0
	...
	for name,group in date_group:
		...
		if (group['overlap']==True).any():
			...
			repeat=1
			w=1
			string="Resort Number"+group.Resort.unique()+ " "  +group['Room Type'].unique()+ "Has overlapping dates or date gaps in Main Tab"
	...
	if refde.empty:
		...
		for name,group in date_group:
			...
			if (group['overlap']==True).any():
				...
				repeat=1
				w=1
				string="Resort Number"+group.Resort.unique()+ " "  +group['Room Type'].unique()+ "Has overlapping dates in FAIRDATES Tab"
				data = pd.DataFrame({"Error": string},index=[0])
				valid=valid.append(data)
				e=2
	##########################################################################################################################################################################################################
	...
	if rebde.empty:
		...
		for name,group in date_group: 
			...
			if (group['overlap']==True).any():
				...
				qa.append(group.Resort.unique())
				repeat=1
				string="Resort Number"+group.Resort.unique()+ " "  +group['Room Type'].unique()+ "Has overlapping dates in BlackoutDates Tab"
				data = pd.DataFrame({"Error": string},index=[0])
				valid=valid.append(data)
				e=2
    ##############################################################################################################################################################################################################	
	...
	if w==0:
		...
		if rebdc_Final.empty:
			rebdc_Final = pd.DataFrame(columns=['6H','Resort','BWI Rate Code','Room Type','Begin Date','End Date','1 Person Rate','2 Person Rate'])
			pass
		else:
		...
		if Fair_Rates_Final.empty:
			Fair_Rates_Final = pd.DataFrame(columns=['7H','Resort','BWI Rate Code','Room Type','Begin Date','End Date','1 Person Rate','2 Person Rate'])
			pass
		else:
		...
		Hotel_Details_Final.columns=['3H','Resort','BWI Rate Codes','Rate Code for Avail','Base Rate','Flat/Percentage']
		RI_Final.columns=['4H','Resort','BWI Rate Code','Room Type','Begin Date','End Date','1 Person rate','2 Person rate']
		Rates_Final.columns=['5H','Resort','BWI Rate Code','Room Type','Begin Date','End Date','1 Person rate','2 Person rate']
		rebdc_Final.columns=['6H','Resort','BWI Rate Code','Room Type','Begin Date','End Date','1 Person rate','2 Person rate']
		rebdc_Final = rebdc_Final.sort_values(by=['Resort', 'End Date'])
		Fair_Rates_Final.columns=['7H','Resort','BWI Rate Code','Room Type','Begin Date','End Date','1 Person rate','2 Person rate']
		...
		Hotel_Details_Final=Hotel_Details_Final[['3H','Resort','BWI Rate Codes','RATE LEVEL','Rate Code for Avail','Base Rate','FORECAST SEGMENT','Flat/Percentage','Rate Code for Confirm','MODIFY_RATE_YN','MODIFY_CONFIRM_YN','MODIFY_INVENTORY_YN','MODIFY_STATUS_YN','ROH_YN']]
		d_Final=d_Final[['2H','Corp Acct #','GDS HOST','GDS Rate Codes','Multi Rate Codes','Resort','BWI Rate Code','Inactive Date']]
		...
		RI_Final.to_excel(writer, sheet_name='Sheet1',startrow=len(H1)+len(d_Final)+len(Hotel_Details_Final)+3,index=False)  # Default position, cell A1.
		Rates_Final.to_excel(writer, sheet_name='Sheet1', startrow=len(H1)+len(d_Final)+len(Hotel_Details_Final)+len(RI_Final)+4,index=False)
		rebdc_Final.to_excel(writer, sheet_name='Sheet1', startrow=len(H1)+len(d_Final)+len(Hotel_Details_Final)+len(RI_Final)+len(Rates_Final)+5,index=False)
		Fair_Rates_Final.to_excel(writer, sheet_name='Sheet1', startrow=len(H1)+len(d_Final)+len(Hotel_Details_Final)+len(RI_Final)+len(Rates_Final)+len(rebdc_Final)+6,index=False)
		Cancel_Policy_Final.to_excel(writer, sheet_name='Sheet1', startrow=len(H1)+len(d_Final)+len(Hotel_Details_Final)+len(RI_Final)+len(Rates_Final)+len(rebdc_Final)+len(Fair_Rates_Final)+7,index=False)
		...
		for i in range(len(H1)+3,len(H1)+len(d_Final)+len(Hotel_Details_Final)+len(RI_Final)+len(Rates_Final)+len(rebdc_Final)+len(Fair_Rates_Final)+len(Cancel_Policy_Final)+9):
